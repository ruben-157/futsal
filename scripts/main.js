...
function renderMvpChaseCard(){
  ...
  title.innerHTML = '<span class="label-full">Closest to Most Valuable Player Badge</span><span class="label-short">Closest to MVP</span>';
...
async function populateMvpChaseCard(refs){
  ...
  try{
    ...
    const thead = document.createElement('thead');
    thead.innerHTML = '<tr><th>Player</th><th><span class="label-full">Current Points per Session</span><span class="label-short">Current PTS/S</span></th><th><span class="label-full">PTS Needed</span><span class="label-short">PTS Needed</span></th></tr>';
    const tbody = document.createElement('tbody');
    const toShow = result.rows;
...
let modalCtx = null; // { matchId, aId, bId, round }
  ...
  function renderLiveScore(){
    ...
    // Helper to render per-stat delta from current match
    const appendDelta = (cell, delta)=>{
      if(!delta) return;
      const badge = document.createElement('span');
      badge.className = 'live-score-delta';
      if(delta < 0) badge.classList.add('neg');
