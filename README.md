...
- Badges: up to two pill badges appear next to each player — ⭐ Latest Top Scorer (led the most recent session in goals), 🎖️ Playmaker (highest points+goals contribution in the latest session), 🥇 All-Time Topscorer (most career goals), 🏆 Session Ace (most sessions ending with the day’s highest points), 🏅 Most Valuable Player (highest Pts/Session with ≥60% attendance), 🔥 Hat Trick (3 straight scoring sessions), 🛡️ Iron Man (6 straight attendances), 🎯 Sharpshooter (2+ goals/session), ⚡ On Fire (biggest positive form jump), and 📈 Rocket Rank (5+ rank jump). The modal lists all earned badges with descriptions.
